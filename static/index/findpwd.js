...
$(function() {
	$(".captcha").click(function() {
		$(this).attr("src", "/captcha/refresh?" + Math.random()), $("#captcha").val("").focus()
	}), $("form").submit(function(a, t) {
		var c = $("#email"),
			o = $("#captcha");
		return /^(\w)+(\.\w+)*@(\w)+((\.\w{2,3}){1,3})$/.test(c.val())?4!==o.val().length ? BootstrapDialog.alert("请输入正确的验证码！", function() {
			o.focus()
		}) : $.ajax({
			url: "/emailcodes/",
			type: "POST",
			...
			success: function(a, t) {
				a.status === t && BootstrapDialog.alert("提交成功，请注意查收邮件！", function() {
					location.reload()
				})
			},
			error: function(a, t) {
				BootstrapDialog.alert("提交失败，请重新输入！"), $(".captcha").trigger("click"), o.focus()
			}
		}) : BootstrapDialog.alert("请输入正确的邮箱！", function() {
			c.focus()
		}), !1
	})
}); +
