...
function loadMap(e){var t=document.getElementById("map");t.parentNode.removeChild(t);var r=["#1f77b4","#ff7f0e","#2ca02c","#9467bd","#8c564b","#e377c2","#7f7f7f","#bcbd22","#17becf"],a=e.width,n=e.height,o=d3.select("#mapRow").append("svg").attr({id:"map",width:a,height:n}),c=o.append("g"),s=d3.behavior.zoom().on("zoom",function(){c.attr("transform","translate("+d3.event.translate+") scale("+d3.event.scale+")")});o.call(s),$(".courses-list").fadeOut();var i=$(".courses-map").slideDown();e.course?i.attr("data-id",e.course):e.course=i.attr("data-id"),d3.json("/neo4j/course/"+e.course,function(t){if(0!=t.nodes.length){var s={},i=[];t.nodes.map(function(t){void 0===s[t.id]&&(t.course=t.properties.course,t.course==e.course?(t.index=0,t.x=a/2-100,t.y=n/2-50,t.fixed=!0):t.index=1,s[t.id]=i.length,i.push(t))});var l={},u=[];t.rel.map(function(t){if(void 0==l[t.startNode+t.endNode]){var r=i[s[t.startNode]],a=i[s[t.endNode]];r.course==e.course?a.index=2:a.course==e.course&&(r.index=2),u.push({type:t.type,target:s[t.endNode],source:s[t.startNode],desc:t.properties.desc}),l[t.startNode+t.endNode]=!0}}),getCourse(e.course);var d=d3.layout.force();d.on("tick",function(){c.selectAll("line.link").each(function(e){var t,r,a,n,o=d3.select(this);if("NEXT"==e.type){var c=e.target.x-e.source.x,s=e.target.y-e.source.y,i=Math.sqrt(c*c+s*s),l=c/i,u=s/i,d=35,p=35;t=e.source.x+d*l,r=e.target.x-p*l,a=e.source.y+d*u,n=e.target.y-p*u,o.attr("marker-end","url(#arrow)")}else t=e.source.x,a=e.source.y,r=e.target.x,n=e.target.y;o.attr("x1",t),o.attr("x2",r),o.attr("y1",a),o.attr("y2",n)}),c.selectAll("g.node").selectAll("circle.ring").attr({cx:function(e){return e.x},cy:function(e){return e.y}}),c.selectAll("g.node").selectAll("circle.outline").attr({cx:function(e){return e.x},cy:function(e){return e.y}}),c.selectAll("g.node").selectAll("text").attr({x:function(e){return e.x-35},y:function(e){return e.y-35}}),p.attr({x:function(e){return(e.source.x+e.target.x)/2-25},y:function(e){return(e.source.y+e.target.y)/2+5},transform:function(e){var t=e.target.x-e.source.x,r=e.target.y-e.source.y,a=360*Math.atan(r/t)/(2*Math.PI),n=(e.target.x+e.source.x)/2,o=(e.target.y+e.source.y)/2;return"rotate("+a+","+n+","+o+")"}})}).charge(-1300).linkDistance(200).nodes(i).links(u).size([a,n]).alpha(.1),o.append("svg:defs").append("svg:marker").attr("id","arrow").attr("viewBox","0 -5 10 10").attr("refX",6).attr("markerWidth",5).attr("markerHeight",5).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5").attr("fill","#6ac6ff"),c.selectAll("line.link").data(u).enter().append("line").attr("class","link");var p=c.selectAll("link.desc").data(u).enter().append("text").attr("class","desc").text(function(e){return e.desc}),f=c.selectAll("g.node").data(i),g=f.enter().append("g").attr("class",function(e,t){return 0===t?"node active":"node"}).on("click",function(e){var t=d3.select(this);"node"===t.attr("class")&&(d3.selectAll(".node").attr("class",function(t){return t.course===e.course?"node active":"node"}),getCourse(e.course))}).on("dblclick",function(t){e.course=t.course,loadMap(e)});g.append("circle").attr({r:29,"class":"ring"}),g.append("circle").attr({r:25,"class":"outline"}).style({fill:function(e){return r[e.index]},stroke:"#5CA8DB","stroke-width":"2px"}),g.append("text").text(function(e){return e.properties.name}).style("fill","black"),d.start();for(var h=0;50>h;h++)d.tick();var v=setInterval(function(){d.alpha()<.01?clearInterval(v):d.tick()},80)}})}function getCourse(e){$.getJSON("/api/course/"+e,function(e){var t=$("#course-card").removeClass("hide").css("transition","all .5s").addClass("trans");setTimeout(function(){$(".course-name").text(e.course.name).prepend('<i class="iconfont icon-'+e.course.type+'"></i> ');var r=$(".course-author").attr("href","/personal/"+e.author._id).empty();$("<img>").attr("src",e.author.avatar).appendTo(r),$("<div>").append($("<div>").text(e.author.name).prepend($("<i>").addClass("glyphicon glyphicon-user"))).append($("<div>").text(e.author.job).prepend($("<i>").addClass("glyphicon glyphicon-briefcase"))).appendTo(r),$("#goMore").attr("href","/course/"+e.course._id),$("#goClass").attr("href","/class/"+e.course._id),$(".course-desc").text(e.course.description),t.removeClass("trans").draggable({cursor:"move",containment:"#menuNav"})},700),setTimeout(function(){t.css("transition","background-color .6s").draggable({cursor:"move",containment:"#menuNav"})},1300)})}$(function(){ejs.open="##",ejs.close="##";var e=document.getElementById("tpl-s-list"),t=document.getElementById("view-subject"),r={};$("#navMap").click(function(){$.getJSON("/subject",function(a){t.innerHTML=ejs.render(e.innerHTML,{subjects:a}),$(".subject").click(function(){$("html,body").animate({scrollTop:0},500),r.width=t.offsetWidth,r.height=t.offsetHeight,r.course=$(this).attr("data-base"),loadMap(r)})})}),$("#backToNav").click(function(){$(".courses-map").slideUp(),$(".courses-list").fadeIn()}),$("#refreshMap").click(function(){loadMap(r)})});
